...
const Footer: React.FC = () => {
  return (
    <footer className="bg-primary text-[#A7A9AE] py-16 md:py-24 relative">
      <div className="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="md:max-w-md lg:h-[304px] lg:w-[426px]">
            ...
            <div className="flex space-x-4">
              {socialLinks.map(({ href, src, alt }, index) => (
                ...
                >
                  <Image src={src} alt={alt} width={32} height={32} className="w-6 h-6 sm:w-8 sm:h-8" />
                </Link>
              ))}
            </div>
          </div>
        ...
        {/* Bottom Section */}
        <div className="border-t border-[#E3E3E3] border-opacity-20 pt-6 sm:pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
            <div className="text-xs sm:text-sm text-center md:text-left">
              <p>
                © 2024 PEP Express Transport Systems. All rights reserved.
              </p>
            </div>
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-6 text-xs sm:text-sm">
              {["Terms of Service", "Privacy Policy", "Cookies"].map((label, i) => (
      ...
      <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-50 flex items-center gap-2 sm:gap-3">
        {/* Chat Button */}
        <button
          className="bg-white text-[#18274B] px-3 sm:px-4 py-2 sm:py-3 rounded-sm shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 sm:gap-3 group"
          style={{
            minWidth: "200px",
            maxWidth: "299px",
            height: "40px",
        ...
        >
          {/* <Image src={arrowUp} alt="Scroll to top" width={48} height={48} /> */}
        </button>
      </div>
    </footer>
  );
